...
@pytest.fixture
def fake_log_box(mock_gui_handler, qtbot):
    test_box = LogBox(QtWidgets.QWidget(), mock_gui_handler)
    qtbot.addWidget(test_box)
    return test_box


def test_start_up_greeting_in_log_box(fake_log_box):
    assert fake_log_box.log_output_box.toPlainText() == GREETING


def test_log_box_updated_with_log_message(fake_log_box, mock_gui_handler):
    log_record = logging.LogRecord(
        "Info", logging.INFO, "", 0, "Hello test", None, None
    )
    mock_gui_handler.emit(log_record)
    full_log = fake_log_box.log_output_box.toPlainText()
